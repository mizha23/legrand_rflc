    ...
    async def session(self):
        try:
            ...
            if self._writer is not None:
                _logger.debug("[aio] Requesting SystemInfo to trigger MAC address...")
                await self.send({"Service": "SystemInfo"})
...
class _ConnectionContext(contextlib.AbstractAsyncContextManager):
    def __init__(self, host: str, port: int):
        ...
        self._port = port
        self._writer = None
        _logger.debug("[aio] _ConnectionContext initialized for %s:%s", host, port)

    async def __aenter__(self):
        _logger.debug("[aio] _ConnectionContext about to open connection...")
        reader, self._writer = await asyncio.open_connection(
            self._host, self._port
        )
        _logger.debug("[aio] _ConnectionContext connection opened successfully.")
        return (reader, self._writer)

    async def __aexit__(self, et, ev, tb):
        _logger.debug("[aio] _ConnectionContext closing connection...")
        self._writer.close()
        try:
            await self._writer.wait_closed()
            _logger.debug("[aio] _ConnectionContext connection closed.")
        except OSError:
            _logger.debug("[aio] _ConnectionContext OSError on close.")
...
class Connector(Authenticator):
    ...
    async def loop(self):
        ...
        before trying again."""
        _logger.debug("[aio] Connector.loop() started")
        self._task = asyncio.current_task()
        count = 0
        try:
            while True:
                try:
                    _logger.debug("[aio] Entering async with _ConnectionContext")
                    async with _ConnectionContext(
                        self._host, self._port
                    ) as connection:
                        # connection success, will be closed with context exit
                        _logger.debug("[aio] Inside _ConnectionContext, connection successful")
                        count = 0
                        self._reader, self._writer = connection
                        _logger.debug("[aio] Emitting EVENT_CONNECTED")
                        await self._emit(self.EVENT_CONNECTED)
                        try:
                            _logger.debug("[aio] About to await self.session()")
                            res = await self.session()
                            _logger.debug("[aio] self.session() completed with result: %s", res)
                            return res
                        except EOFError:
                            _logger.warning("EOFError in session")
                        except OSError as error:
                            _logger.warning("OSError in session %s", error)
                        except asyncio.TimeoutError:
                            _logger.warning("asyncio.TimeoutError in session")
                        finally:
                            _logger.debug("[aio] Clearing reader/writer and emitting EVENT_DISCONNECTED")
                            self._reader, self._writer = None, None
                            await self._emit(self.EVENT_DISCONNECTED)
                except OSError as error:
                    # connection error
                    _logger.debug("[aio] OSError caught connecting: %s", error)
                    if self._loop_timeout < 0:
                        _logger.debug("[aio] loop_timeout < 0, raising OSError")
                        raise
                    _logger.warning("OSError %s", error)
                    # reconnect after exponential backoff
                    _logger.debug("[aio] Sleeping for %s seconds before retry", min(2 ** count, self._loop_timeout))
                    await asyncio.sleep(min(2 ** count, self._loop_timeout))
                    count += 1
        finally:
            _logger.debug("[aio] Connector.loop() exiting, clearing _task")
            self._task = None
